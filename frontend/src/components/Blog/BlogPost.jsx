...
const BlogPost = ({ post, user, onLike, onPostUpdated }) => {
  ...
  useEffect(() => {
    if (user && post.liked !== undefined) {
      ...
      // For existing implementation, we can check if the user has liked the post
      // This would require an API call to check, but for now we'll rely on the post.liked field
      setIsLiked(false);
    }
  }, [user, post.liked]);
