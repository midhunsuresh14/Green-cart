...
export default function QuickActions({ onAddUser, onAddProduct, onAddRemedy, onViewOrders }) {
  return (
    <Card elevation={0} sx={{ borderRadius: 3, border: '1px solid', borderColor: 'divider', mt: 3 }}>
      ...
      <CardContent>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={3}>
            <Button fullWidth variant="contained" color="warning" startIcon={<LocalPharmacyIcon />} sx={{ borderRadius: 3, py: 2 }} onClick={onAddRemedy}>Add Remedy</Button>
          </Grid>
          <Grid item xs={12} md={3}>
