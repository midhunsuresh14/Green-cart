...
test.describe('Shopping Cart Management Functionality', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto('/');
    await page.evaluate(() => {
      // Clear all cart-related keys using the same pattern as App.js
  ...
  test('should calculate total price correctly', async ({ page }) => {
    ...
    // Logic from ShoppingCart.js:
    // Subtotal: 2*100 + 1*200 = 400
    // Discount: 0 (subtotal <= 999)
    // Shipping: 49 (subtotal < 499)
    // Tax: (400 - 0) * 0.08 = 32
    // Total: 400 - 0 + 49 + 32 = 481

    const subtotalText = await page.locator('text=400').or(page.locator('text=₹400')).first().isVisible();
    const totalText = await page.locator('text=481').or(page.locator('text=₹481.00')).first().isVisible();

    // Verify total section exists and shows the price
    await expect(page.locator('.order-summary')).toBeVisible({ timeout: 3000 });
    expect(subtotalText || totalText).toBeTruthy();
  });
